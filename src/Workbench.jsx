  ...
  return <section className={page === 'review' ? 'workbench wb-fill' : 'workbench'}>
    ...
    {page === 'checkpoints' && (!cwd ? <p>先打开一个项目。</p> : <>
      ...
      <div className="wb-records">{checkpoints.map(c => <article key={c.id} className={`wb-record${restore?.id === c.id ? ' open' : ''}`}>
        <div className="wb-record-main"><div><b>{c.label}</b><small>{when(c.at)} · {c.count} 个文件</small></div><div className="wb-actions"><button className="btn ghost" disabled={busy || projectRunning} onClick={() => { if (restore?.id === c.id) { setRestore(null); return; } act(async () => setRestore(await api.previewRestore({ cwd, id: c.id })), '正在对比文件…'); }}>恢复</button><button className="btn danger" disabled={busy || projectRunning} onClick={() => act(async () => {setCheckpoints(await api.deleteCheckpoint({cwd,id:c.id})); setRestore(null);})}>删除</button></div></div>
        {restore?.id === c.id && <div className="wb-restore" ref={restoreBox}>{restore.files.length ? <ul className="wb-restore-files">{restore.files.map(f => <li key={f.path}><em>{f.action}</em><span>{f.path}</span></li>)}</ul> : <p>没有需要改回的文件。</p>}<div className="wb-actions">{restore.files.length > 0 && <button type="button" className="btn primary" disabled={busy || projectRunning} onClick={() => act(async () => { const r = await api.restoreCheckpoint({ cwd, ...restore }); if (!r.canceled) { await load(); } })}>确认恢复</button>}<button type="button" className="btn ghost" disabled={busy} onClick={() => setRestore(null)}>取消</button></div></div>}
      </article>)}</div>
