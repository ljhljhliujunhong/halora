...
const iconFile = fs.existsSync(path.join(root, 'build/launcher.ico')) ? 'launcher.ico' : 'icon.ico';
const built = spawnSync(compiler, ['/nologo', '/target:winexe', '/platform:anycpu', `/out:${launcher}`, '/r:System.Windows.Forms.dll', '/r:System.Web.Extensions.dll', `/win32icon:${path.join(root, 'build', iconFile)}`, path.join(root, 'scripts/launcher.cs')], { windowsHide: true, encoding: 'utf8' });
